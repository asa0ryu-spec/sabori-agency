...
app.get('/image/:filename', async (c) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // トークンの先頭と長さを表示して確認
      const tokenDebug = token ? `${token.substring(0, 4)}... (Length: ${token.length})` : 'NONE';
